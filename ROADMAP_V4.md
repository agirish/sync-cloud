...
| **The status bar's chord** | **⌘/ , matching Finder.** `shortcutsReference` moves to **⇧⌘/**. | ⌘/ is `shortcutsReference` today (`AppChord.swift:51`). ⇧⌘/ is free and is macOS's own Help-menu convention, so nothing else in the registry moves. |
