...
| **The field disappearing while the list is up** | **The palette closes.** | Decided on review 2026-08-19. macOS folds a toolbar item behind the overflow chevron on a narrow window, and `goToFieldItemView` refuses a folded item's view (it answers in its own bounds and hands back a plausible rect at the window's corner). The anchor then answers nil forever and the panel stayed exactly where it was last placed — a list hanging under nothing. The same exhaustion covers the field never mounting at all, which used to leave an invisible inert panel and an open toolbar item with no list beneath it. The palette is the field plus its list; without the field there is no palette. |
