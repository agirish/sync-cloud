...
@Suite struct SettingsSearchTests {
    ...
    /// The loose-files inbox went unindexed from the day it shipped: searching "inbox", "loose
    /// files" or "TODO" returned nothing while the control sat on the Tidy tab. It is the one
    /// setting whose *value* is the word people would search ("TODO"), and the title spells
    /// "Loose-files" hyphenated, so the obvious queries all need a keyword to land.
    @Test func looseFilesInboxIsFindableByTheWordsItsControlUses() {
        for query in ["inbox", "loose files", "todo", "default folder", "filing"] {
            let results = filterSettings(SettingsSearchIndex.all, query: query)
            #expect(results.contains { $0.title == "Loose-files inbox" && $0.tab == .tidy },
    ...
    @Test func everyControlLabelInTheTabSourcesIsIndexed() throws {
        ...
        // labels, so losing that vehicle entirely drops the total from 48 to 46 and every
    ...
    ///   row title; "Duplicates" and "Filing" head the two halves of the Tidy tab. (Headers that
    ///   *are* their section's only control — "Tint", "List density", "Cloud spend" — are indexed
    ///   like any other control and are not listed here.)
    /// - **Readouts** — a `SettingsRow` showing a figure with nothing to change. All four are in
    ///   Tidy's Cloud spend section, which is indexed as a whole.
    ///
    /// Adding a line here is how you say "this label is not a setting". Adding one to silence a
    /// real control is the failure this test exists to catch, so each entry earns its line — and
    /// all twelve do: drop any one and the scan fails on it.
    ...
    /// Every control label written into the Settings sources, from the vehicles that carry one:
    /// `SettingsRow("…")`, `SettingsSection("…")`, and `Toggle`'s label in both its string and its
