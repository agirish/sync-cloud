    ...
    public func mergeDuplicateGroup(_ group: DuplicateGroup) async -> Bool {
        ...
        if anyPermanentlyDeleted {
            ...
            Logger.shared.warning(
                "Tidy: “\(group.name)” had a redundant copy deleted permanently rather than trashed — the merge is not undoable, and undoing the fold would remove the copied files from \(group.keeper.name)")
        }
        return true
    }
