...
@Suite struct FilingSpendStoreTests {
    ...
    @Test func everyWritePostsTheChangeSignal() {
        // The signal exists because per-surface refresh lists do not compose across windows.
        // Settings caches the spend figures and refreshes on the three moments it can name —
        // appearing, its own history sheet closing, its own Clear. `FilingSpendHistoryView` is
        // also presented from the Organize lens, and its Clear History is a fourth moment that
        // list could not have named. Both writers must post, or a cached reader is exactly one
        // unlisted writer away from stale.
        let (defaults, name) = suite()
        defer { wipeDefaultsSuite(name) }
        ...
        // process-wide, so an unscoped observer counts the other tests' posts too.
