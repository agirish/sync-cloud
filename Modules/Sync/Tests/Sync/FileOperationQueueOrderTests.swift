    ...
    @Test func theOperationRequestedSecondNeverRunsFirstOrAlongsideTheFirst() async throws {
        ...
        for _ in 0..<trials {
            ...
            if seen != ["in1", "out1", "in2", "out2"] {
                if seen.first == "in2" { inversions += 1 } else { overlaps += 1 }
            }
            if maxInside.withLock({ $0 }) > 1 { overlaps += 1 }
        }
        ...
        #expect(overlaps == 0,
                "two operations queued back-to-back overlapped in \(overlaps) of \(completed) pairs — the queue is not serializing them")
    }
}
