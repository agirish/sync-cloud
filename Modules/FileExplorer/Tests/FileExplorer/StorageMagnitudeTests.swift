...
@Suite(.machinePinned(.pixelSampling)) struct StorageMagnitudeRenderTests {
    ...
    static func entries() -> [StorageEntry] {
        ladder.enumerated().map { index, bytes in
    ...
    /// **One list at a time, by emptying the others.**
    ///
    /// This line has no section rail, so a page cannot be narrowed to one list the way it can on
    /// `main` — but the same question is answerable by handing the report only the list under test.
    /// The empty ones draw their own "nothing here" text and no rows, so every bar on the canvas
    /// belongs to the list named.
    static func report(largest: [StorageEntry] = [], stale: [StorageEntry] = [],
                       reclaim: [StorageEntry] = []) -> StorageLensReport {
        // **No treemap.** This page always draws one — there is no rail here to narrow it away —
        // and the treemap is chromatic across the full width, so `barEdges` would read its right
        // edge as a bar on every fixture, including the one that must have none.
        StorageLensReport(treemap: [], largest: largest, stale: stale, reclaimCandidates: reclaim,
    ...
    private func render(_ report: StorageLensReport) -> NSBitmapImageRep? {
        let manager = FileSyncManager()
        manager.storageLensReport = report
        let subject = StorageLensView(syncManager: manager, providerName: "Projects",
                                      onBuild: {}, onReveal: { _ in })
    ...
    @Test func theFourBarsStandInTheRatioOfTheFilesTheyMeasure() throws {
        let rep = try #require(render(Self.report(largest: Self.entries())),
                               "the largest list did not render")
    ...
    @Test func theAgeOrderedListDrawsNoBars() throws {
        let stale = try #require(render(Self.report(stale: Self.entries())))
        let largest = try #require(render(Self.report(largest: Self.entries())))
