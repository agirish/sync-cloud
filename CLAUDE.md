...
**Open, deliberately deferred: `v3.x` has none of the `DeleteOutcome` family**, so it cannot tell
a trashed item from a permanently deleted one and offers ⌘Z for removals it cannot take back. The
three fixes, what 3.x still does without them, and why this is a scope call rather than a
cherry-pick are in [`docs/backports.md`](docs/backports.md) — which is also where any other
line's debt is recorded, and where a decision gets written down when one is settled.
...
silently dropping `.rounded` and resolving to the default face, fixed in `97154a32`.
