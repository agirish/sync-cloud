...
enum FilingArtifacts {
    ...
    ///
    /// Returns whether anything was attached, so a caller that has just written a profile can tell
    /// "the walk landed and is in use" from "the walk landed and something else is still active".
    @discardableResult
