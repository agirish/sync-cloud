            ...
            Topic(id: "editor-workspace", title: "Write in a text file", systemImage: "square.and.pencil", article: Article(
                ...
                blocks: [
                    .bullets([
                        ...
                        "Autosave can be switched off for one file at a time — the Autosave control sits on the line under the file name, beside the word that says where your typing is. Off, nothing is written until you press ⌘S, and leaving the file asks whether to save it, the way documents used to. It is per file and it lasts for as long as the app is running: close the file and come back and it is still off, but quitting SyncCloud puts autosave back on everywhere, so the safe setting is always the one you start from.",
                        "A dot beside the file name and a word under it tell you where your typing is. While you type, the dot is on and the line says “unsaved”; a second or two after you stop, the dot goes out and it says “saved” — that is the write having landed. If the dot ever turns amber and the line says “not saving”, nothing you type is reaching the file until you settle it — see below.",
                        ...
                        "The line under the document counts its words and characters, says where the caret is, and names the file's text encoding and its line endings — so a file that came from Windows says CRLF rather than being quietly rewritten.",
