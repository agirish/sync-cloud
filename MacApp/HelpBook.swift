    ...
    static let sections: [Section] = [
        Section(title: "Getting started", topics: [
            Topic(id: "setup", title: "Set up SyncCloud", systemImage: "checklist", article: Article(
                intro: "Setup is ten screens: five short questions, then a look at what SyncCloud made of your folders. It asks for the handful of things the app cannot work out on its own — which of the places on this Mac you use, which folder to read, your name as documents print it, who else your documents belong to, and which short folder names are countries. It runs itself once on a new install, and Help ▸ Set Up SyncCloud… opens it again any time.",
                blocks: [
                    .paragraph("The order matters: SyncCloud reads your folder early, on screen 2, so the three questions after it can be asked with your own folder names in front of you rather than in the abstract. Reading is names only and takes seconds — no document is opened at that stage."),
                    .bullets([
                        "1 Locations — everything found in the system's cloud-storage folder, plus any folder you add. Everything arrives switched on; switch off what you don't use. Settings calls this tab Sources, and it is the same list.",
                        "2 Learn — the folder SyncCloud reads. One folder, not a whole account: Documents is usually the right one. Skipping is allowed, and then Organize files by folder name alone.",
                        "3 You — your name, and the forms of it a document might print. Order matters, so surname-first is offered as its own form; initials are offered and left unticked. The surname is only used to build the forms — it is not saved on its own.",
                        "4 People — anyone else whose documents live in your folders. SyncCloud proposes the names it found — a folder sitting directly under one that says what its children are, like a person's folder under Family — and each chip carries the folder that vouches for it. Nothing is ticked to begin with, and you can type a name that has no folder at all.",
                        "5 Countries — the short folder names that might be country codes. A name is ticked for you only when it is a real country code and it splits several different parent folders; everything else is offered unticked, because a name like IT is as likely to be a department.",
                    ]),
                    .paragraph("Then Structure: the tree SyncCloud read, with its reading of each folder beside it — “a year”, “files go here”, “inbox: nothing will be filed here”. Nothing has been written at this point, so going back to fix a name, a country or the folder itself costs nothing. Save writes the folder profile: the record of how your tree is shaped, and the thing that lets Organize propose a destination for a loose file and a better name for a badly named one. It takes effect straight away, with no relaunch."),
                    .paragraph("Save can also start reading your documents — page one of each, in the background, over a few hours. That is optional, the switch says so before you press the button, and it can be paused or stopped from any Organize lens."),
                    .paragraph("The last two screens decide nothing: Workspaces says what to do next, and Appearance offers text size, light or dark, and an accent. Nothing here is locked in — every answer has a home in Settings, and setup is a faster way to give them all at once rather than the only way. Settings ▸ General has “Run setup again…”; Settings ▸ Advanced has “Reset setup…”, which forgets that setup has run so it offers itself again on the next launch."),
            ...
            Topic(id: "browse-workspace", title: "Browse your files", systemImage: "folder", article: Article(
                ...
                blocks: [
                    .bullets([
                        ...
                        "A status bar sits at the foot of the pane: how many items the tree holds, what your selection comes to, how many files are still in the cloud, and when the listing was read. Turn it off with View ▸ Status Bar. As the pane narrows the line drops facts from the right, keeping the item count longest.",
                        "The cloud-only count reads “—” for a while after the tree loads. Finding it means asking the filesystem about every file, so it is counted in the background rather than making you wait for the folder — and a dash is shown instead of a number still climbing towards the answer.",
                        ...
                        "To compare two files that are not duplicates of each other, select one in a pane and right-click the other: the menu offers “Compare with ‘…’ from ⟨the other pane⟩”, and opens the same side-by-side viewer. Two files picked in one pane offer “Compare these two files”. Both are read-only — the panes already own copying, moving and deleting.",
                        "The rest of a row's right-click menu is in the File menu for the same reason — Open in Edit, Open in New Tab, Quick Look, Download, Reveal in Finder, Rename, Copy to… and Move to… all act on the focused pane's selection, so none of them needs you to find the right row to right-click first. File ▸ Download is offered for one file whose content is still in the cloud, and asks iCloud to fetch it the way the row's own Download does; for other providers Reveal in Finder is the way. Open in Edit is the one of them with a key, ⌘O, which nothing else in the app had claimed. Rename also answers to ↩ with a row selected, the way it does in Finder; the menu item carries no key because a bare ↩ registered there would take the Return that commits every sheet in the app.",
                    ]),
            ...
            Topic(id: "editor-workspace", title: "Write in a text file", systemImage: "square.and.pencil", article: Article(
                ...
                blocks: [
                    .bullets([
                        "The rail lists the text files in the folder the file pane on its left is showing — Markdown, plain text, and the other text kinds SyncCloud already reads. That pane starts folded to a narrow strip; click it open to browse anywhere, or use the sidebar to jump. Open that pane and the list steps aside: the pane is the list, files Edit cannot open are dimmed, and one click opens a file.",
                        "There are six ways to bring a file here. Every one but Compare's list of differences also takes the file pane on the left to the file's folder, when that folder is in the source the pane is showing — a file from anywhere else opens and the pane stays put; that list leaves the pane where it is, because in Compare it is half of the comparison, and runs the comparison again when you come back to it after editing the file. Four are wherever a file pane lists it — in Browse, either Compare pane or the columns view: right-click a text file and choose Open in Edit, the first thing that menu offers; with the preview showing, click the Edit button by its name (beside it, or under it when the name is long); choose Open in Edit first in the Info inspector's row of actions (⌘I); or choose File ▸ Open in Edit — ⌘O — for the one text file selected in the focused pane. The other two are right-click menus elsewhere: a row in Compare's list of differences offers Open in Edit once for each side the file is on, and a copy in Organize's duplicates offers it first.",
                        ...
                        "Autosave can be switched off for one file at a time — the Autosave switch sits on the line under the file name, next to what kind of file it is. Off, nothing is written until you press ⌘S, and leaving the file asks whether to save it, the way documents used to. It is per file and it lasts for as long as the app is running: close the file and come back and it is still off, but quitting SyncCloud puts autosave back on everywhere, so the safe setting is always the one you start from.",
                        "The dot beside the file name is where your typing is. While you type it is on; a second or two after you stop it goes out, and that is the write having landed. There is no word for either — a header that said “saved” at all times would be saying it whether or not anything had ever been written, and the dot going out is the thing you can actually trust.",
                        "Words appear on that line only when they change what you have to do. If you switch Autosave off, the line says “unsaved — ⌘S”, because nothing is coming to write the file and you are the one who has to. If the dot turns amber and the line says “not saving”, something is in the way and nothing you type is reaching the file until you settle it — see below. And a file that cannot be written at all says “read only”.",
                        ...
                        "A Markdown file gets Source, Preview and Split above it: the text you typed, the rendered document, or both side by side with a divider you can drag. The same three are the first items of the Text menu, ticked for the one showing, on ⌃⌘1, ⌃⌘2 and ⌃⌘3. Other text files are always just the text, so the choice is not offered — the capsule is hidden and Text ▸ Preview and Split are greyed. In Split the preview follows the text as you scroll it.",
                        "⌘P prints what Preview shows: the rendered document, whichever of the three you are looking at — printing a Markdown file means the document, not its markup. File ▸ Export as PDF… writes those same pages to a file instead, offering the folder the rail is reading and the document's own name with .pdf on it. A plain text file has no rendered form, so both print it as text, wrapped to the page. Either way it is what is on screen that is printed, including typing that has not been saved yet.",
                        "⌘F searches the open document when the caret is in it — find, replace, replace all, and a count of the matches as you type. The magnifying glass beside the Source/Preview/Split buttons does the same with the mouse. Anywhere else in SyncCloud ⌘F still finds files in the pane, exactly as it did; it follows the caret rather than the workspace, so ⌘F typed in a pane's own search box is still about files. With the bar open, Text ▸ Find Next (⌘G) steps to the next match and Text ▸ Use Selection for Find (⌘E) puts what you have selected into the search field.",
                        "The Markup menu carries the verbs: Bold, Italic, Strikethrough, Inline Code and Link… on ⌘B, ⌘I, ⇧⌘X, ⇧⌘K and ⇧⌘L, then Heading 1–3 and Body, then Bulleted List, Numbered List, Task Item and Block Quote, then Code Block and Horizontal Rule. Each one works on what you have selected and takes itself off if you choose it again, and ⌘Z steps back through them like any other edit. The same list, in the same order and with the same keys shown, is on the text's right-click menu. Headings and lists have no keys because ⌘1 through ⌘4 are the workspaces.",
                        "The five keys act only while the caret is in the document's text. In the Go to field, the rename row, a pane's search box — or the find bar's own field — they do nothing to the document, and ⌘I goes on meaning the Info inspector there, as it does everywhere outside Edit. Markup ▸ Italic and View ▸ Info Inspector both show ⌘I for that reason: it is one key with two jobs, decided by where you are typing.",
                        "In Preview or Split, the checkbox on a task item is a real checkbox — click it and the file itself changes from - [ ] to - [x]. Links are live too: a web link opens in your browser, and a link to a heading in the same document scrolls to it instead of leaving.",
                        "An image written on a line of its own is drawn, as long as it is a file already on this Mac — beside the document, or at a path you gave. Nothing is downloaded: a web address shows its alt text and says so, and a file still in the cloud is left alone rather than fetched, because opening a note should not pull things onto your disk.",
                        "Two switches say how the text is drawn: Wrap Lines, on to start with, and Check Spelling While Typing, off to start with. Both are on the Text menu and on the text's right-click menu, and both are remembered. Spell checking only underlines — SyncCloud never changes what you typed, so smart quotes, dashes and autocorrect stay off with no way to turn them on. In a file of YAML or shell, a curly quote is a broken file.",
                        "Text ▸ Autosave This File is the header's Autosave switch as a menu item — the same setting, ticked when it is on, and greyed on a file that could not be written anyway. Every item in the Text and Markup menus is greyed unless Edit is on screen with a document open; in Preview, the Markup verbs and the two find items are greyed too, because the rendered page has no text to select in.",
                        "A file that opens with a --- block of settings shows it folded into one Front matter line at the top of the preview, with the number of keys in it. Click it to read them.",
                        "The rail has two tabs: Text Files and Outline. Text Files is the folder's list, with the folder it is reading named on the line under the tabs — that is also the folder ⌘N creates in. Outline is the headings in the open Markdown file: click one to go to it, and the heading you are in is marked as you move about. It keeps its place per file — come back to a document and the outline is where you left it, unless the heading you are in would be off the fold, in which case that is shown instead. A file with no headings says so rather than showing an empty list. The button beside Find above the document hides the list too, for just the text; the strip at the left edge brings it back.",
                        "The line under the document counts its words and characters, says where the caret is, and names the file's size, its text encoding and its line endings — so a file that came from Windows says CRLF rather than being quietly rewritten. Everything measurable about the file is there rather than in the header, which leaves the header to say which file it is and how you are looking at it.",
                        "The funnel button beside the folder name filters the file list by name, for a folder with more files in it than fit. It belongs to the Text Files tab; ⌘N always brings that tab back, since that is where the name field opens.",
                        ...
                        "If the file changes on disk while you have it open — or an Organize run files it somewhere else — saving would either throw away what arrived or leave a second copy at the old path. So the editor stops saving and asks you straight away which version wins: keep yours (Save Anyway), or take the one on disk and lose what you typed (Reload from Disk). A file that has been moved or renamed is not offered the reload, because there is nothing left at that path to read. If you dismiss the question it stays stopped rather than asking again every couple of seconds; ⌘S brings it back when you are ready to decide.",
                        "You do not have to answer that question blind. For a file that changed, the question has a fourth button — Show What Changed… — which puts your version and the one on disk side by side, with what differs picked out line by line and word by word, and the same three answers along the bottom. Both sides can be selected and copied, which is often what you actually want: the usual case is one changed line on each side, where neither version is simply right and the honest answer is to merge them by hand. Nothing there writes anything until you press one of the three. A file that has moved is not offered it, for the same reason it is not offered the reload — there is nothing at that path to show.",
                        "Once the editor has stopped, the amber “not saving — changed on disk” words on the line under the file name are a button: click them to open that same comparison again. Dismissing the question with Escape or Cancel leaves the file stopped and does not change anything, so this is the way back to it without pressing ⌘S. The words are only a button for a file that changed — for one that has been moved, or for a write that failed outright, they are just the state.",
                        "⌘Z inside the editor takes back your typing. It is a separate history from the one that takes back file operations, so an undo here can never reach into a move or a copy. Each file keeps its own history while you work, so switching to another file and back leaves ⌘Z still able to take back what you did — the last several files you edited, for as long as the app is running. A file whose contents changed since you left it starts fresh instead, because an undo made against text that is no longer there would put back the wrong thing.",
                        "Right-click the file name at the top for Reveal in Browse: it takes you to the file where it lives, in Browse, without closing it here. Right-click a file in the Text Files list for the same, and for Get Info, Reveal in Finder and Quick Look — each about the file you clicked, not the one that is open.",
                        "The line under the file name says where it lives. With the file pane open it reads, say, “in Finance” — click it and the pane goes to that folder with the file selected. With the pane folded away it spells out the whole path, “iCloud › Documents › Finance”, and each part moves the folded pane there without opening it. The Text Files list follows it; under Just the text, where neither is on screen, nothing visible changes until you bring the list or the pane back, and it is there when you do. A long path folds its middle into “…”, which lists the folders it hides, each one a click away. A file in another of your sources — the right-hand copy from Compare's list of differences, say — is named from that source, “in Dropbox › Backup”, as words only, since the pane is on a different source.",
                        "The ＋ on the file name's row does what ⌘N does, in the folder its tooltip names — and it is there whether the rail, the file pane or neither is showing, and with no file open, so a new file never needs the key.",
                        "The × at the right-hand end of that row closes the file, and so does File ▸ Close Document — with no key, because ⌘W closes the tab. Closing saves the way leaving a file always does, and asks first only when autosave cannot write it; Cancel leaves the file open. The folder, the file pane and Just the text stay as they were, and clicking the file again opens it again. The empty page keeps its header: “No document open”, the ＋, and the folder a new file would be made in.",
                    ]),
                ...
                blocks: [
                    ...
                    .legend([
                        ...
                        LegendItem(systemImage: "clock", mood: .neutral, title: "Recents",
                                   detail: "The folders you were last in, newest first — never one the two sections above already list."),
                    ]),
                    ...
                    .paragraph("A folder is only ever on the column once. Visiting somewhere Favorites or Locations already holds — your Documents folder, or an account's own top level — writes no Recents row, so the eight it shows are eight places nothing else on the column will take you."),
                    .bullets([
                        ...
                        "“Remove from Recents”, on a recent, takes that row off the column. Going back to the folder puts it there again — Favorites is the list that keeps a folder for good.",
            ...
            Topic(id: "scan", title: "Scan for differences", systemImage: "arrow.triangle.2.circlepath", article: Article(
                ...
                blocks: [
                    .bullets([
                        ...
                        "Large trees scan in parallel; the ↻ rung in each pane's header turns into Stop while the scan runs, and the differences count says when it last finished.",
            ...
            Topic(id: "copy-move", title: "Copy and move", systemImage: "arrow.left.arrow.right", article: Article(
                ...
                blocks: [
                    .bullets([
                        ...
                        "A folder that already exists where you are copying or moving one can be merged rather than replaced. Merge is the default button: everything already in the existing folder stays, anything missing is added, and each file that is in both folders is asked about on its own, with “Apply to all” covering the rest of that merge. Replace sends the existing folder to the Trash instead. Before you choose, the alert counts what each answer would do, unless the folders are too large to count quickly. A move merge sends the emptied source folder to the Trash, and leaves it where it was if anything in it was skipped. A package — an app, a photo library, a document your Mac shows as a single file — is one item, never merged.",
                        "Cancel in the progress window stops the item being copied as well as the ones after it, even when that item is one very large file or folder. Nothing half-copied is left behind: the copy is abandoned before any of it reaches the destination. When one item takes more than a moment to copy, the window shows how much of it has been copied. A bulk sync does not show this, because several copies run at once.",
                    ]),
            ...
            Topic(id: "organize-workspace", title: "The Organize workspace", systemImage: "folder.badge.gearshape", article: Article(
                ...
                blocks: [
                    .bullets([
                        ...
                        "Storage — what is taking the space, and where. It reports and proposes nothing.",
                    ]),
            ...
            Topic(id: "tidy-duplicates", title: "Clear out duplicates", systemImage: "doc.on.doc", article: Article(
                ...
                blocks: [
                    .bullets([
                        ...
                        "To compare two files anywhere in Browse or Compare: select one and press Compare with… in the action bar, or ⇧⌘C, or right-click it and choose “Compare with…”. Then click the file to compare against — either pane, any folder, and navigating to find it is expected. Click the marked file again, or press esc, to call it off.",
                        ...
                        "Two PDFs or two images scroll and zoom together, and hold ⌥ to move one pane on its own until you let go.",
                        "Paging is the PDF half — two images have no pages to turn. ⇞/⇟ move both sides at once, and scrolling onto another page moves the page strip with you, so 1–4 open on the page you were actually reading rather than the one you started at. ⌥ breaks that too. A strip under the panes marks every page once it has been compared: green where nothing changed, amber where something did, blue where only one document has that page, red where a side could not be rendered. A page with no mark has not been compared yet. Where one document is longer, the shorter side stops at its last page rather than hiding the pages only one file has.",
                        "In a PDF pair, ↑/↓ jump to the next and previous page that differs, rather than to the next page. Pages that have not been compared yet are compared as the search walks them — you can watch the strip fill in — and one press stops after twenty-five of them so a long document pauses rather than hangs; press again to carry on from there. The strip counts what it has judged, not what the document holds: “1 of 2 compared differ”.",
                        "1–4 switch how the two sides are shown: side by side, a draggable swipe divider, a blend, or the pixel difference on black where identical is black and anything changed glows. That last one lines the two pages up first where it can — two scans of the same sheet fed in a degree apart are corrected for skew and offset, and the view says what it corrected. Where the match is not sharp enough to trust, it compares the pages as they are and says that instead, so it never claims an alignment it did not manage.",
                        "The difference view outlines each changed region and counts them beside the mode buttons — “1 region differs”. A page changed in more than twelve places is counted but not outlined: at that point the boxes are a mesh rather than something to look at.",
                        "Text, source, CSV and JSON files get a read-only line diff instead. An edited line is one row with both versions and the changed words picked out; ↑/↓ step between changes rather than between lines. A file that is too large, not downloaded, or not really text says which of those it is. Two files too different to line up, and a line too long to compare word by word, are declined up front and say so rather than leaving you watching a spinner.",
                    ]),
            ...
            Topic(id: "command-palette", title: "Go to anything (⌘K)", systemImage: "magnifyingglass", article: Article(
                ...
                blocks: [
                    ...
                    .bullets([
                        ...
                        "A typed path inside another of your sources is delivered rather than refused: the row reads “Open in Dropbox”, and ↩ points the pane at that source and lands it on the folder. Only that pane moves, and the one beside it in Compare stays where it is.",
                        "A path it can’t deliver says which of three things is in the way rather than doing nothing: “Not in any source”, “Backup SSD is not mounted”, or “No folder at that path”. Only the last is a claim that nothing is there.",
            ...
            Topic(id: "keyboard-shortcuts", title: "Keyboard shortcuts", systemImage: "keyboard", article: Article(
                ...
                blocks: [
                    .bullets([
                        ...
                        "⌘+ and ⌘− make the whole app's text bigger or smaller, and ⌘0 puts it back to the default — the same steps as the slider in Settings ▸ Readability, never a size it cannot reach. In Edit's text, ⌘B, ⌘I, ⇧⌘X, ⇧⌘K and ⇧⌘L are the Markup verbs and ⌃⌘1, ⌃⌘2 and ⌃⌘3 pick Source, Preview or Split.",
                    ]),
                    .paragraph("The menu bar carries the rest, a menu per place. File holds New Text File… and New Folder…, the tab items, the verbs that act on the selected row — Download among them, for a file whose content is still in the cloud — then Delete, then Close Document and Save, and Export as PDF… and Print… at the end. Its Ignore in Comparison appears only while you are comparing, because ignoring is a statement about a comparison and Browse has none. Edit holds the file clipboard — Cut, Copy, Paste and Select All — with Find… under them, which searches the open document when the caret is in one and the focused pane otherwise. Go holds ⌘K and the per-pane Back and Forward. Compare holds the four transfers plus Review and Verify. Organize holds its six sections, the verbs that aim them at the selected folder, and Undo This Reorganisation, which is the only item there that needs no selection. Text and Markup are Edit's two menus, greyed until Edit is showing a document: Text holds the Source, Preview and Split modes, Find Next and Use Selection for Find, the Wrap Lines and Check Spelling While Typing switches, and Autosave This File; Markup holds the same verbs as the text's right-click menu, in the same order. View holds the four workspaces and the show/hide switches — View ▸ Hidden Files, View ▸ Info Inspector and View ▸ Status Bar among them, the last of which is Browse's own and greys out elsewhere — and View ▸ Text Size, whose Bigger, Smaller and Default Size step the app's type on the same stops as the Text size slider in Settings ▸ Readability."),
                ],
