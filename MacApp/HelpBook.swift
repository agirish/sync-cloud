            ...
            Topic(id: "tidy-duplicates", title: "Clear out duplicates", systemImage: "doc.on.doc", article: Article(
                ...
                blocks: [
                    .bullets([
                        ...
                        "Two PDFs or two images scroll and zoom together, and ⇞/⇟ page both at once. Scrolling onto another page moves the page strip with you, so 1–4 open on the page you were actually reading rather than the one you started at. Hold ⌥ to move one pane on its own — pages included, until you let go. A strip under the panes marks every page once it has been compared: green where nothing changed, amber where something did, blue where only one document has that page, red where a side could not be rendered. A page with no mark has not been compared yet. Where one document is longer, the shorter side stops at its last page rather than hiding the pages only one file has.",
                        "↑/↓ jump to the next and previous page that differs, rather than to the next page. Pages that have not been compared yet are compared as the search walks them — you can watch the strip fill in — and one press stops after twenty-five of them so a long document pauses rather than hangs; press again to carry on from there. The strip counts what it has judged, not what the document holds: “1 of 2 compared differ”.",
