...
struct DuplicateReviewCoordinator {
    ...
    func trashRightCopy(_ review: DuplicateCompareContext) {
        ...
        Task {
            ...
            // `deleteItems` answers a bare Int on this line — v3.x has none of the `DeleteOutcome`
            // family (recorded in CLAUDE.md), so the log cannot say whether the copy reached the
            // Trash or was destroyed. Unchanged here deliberately: that is a separate backport.
