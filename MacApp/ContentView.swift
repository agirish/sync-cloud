...
struct ContentView: View {
    ...
    /// The presentation actually on screen for this side: the rail's key on a single-source
    /// workspace, the pane's own key in Compare. One resolver, because every surface that asks
    /// "where is this pane" has to ask it of the mode the user is looking at.
    func resolvedViewMode(isLeft: Bool) -> PaneViewMode {
        layoutMode == .singleSource ? railViewMode : paneViewMode(isLeft: isLeft)
    }

    /// Whether that presentation draws the pane's column stack.
    ///
    /// The header's path line, the crumb and quick-jump routing, the Tidy scan target and New
    /// Folder all turn on this one question — see `FileSyncManager.paneLocation(isLeft:drawsColumns:)`.
    /// In Tree the stack is parked, not on screen, and every one of those surfaces that reads it
    /// anyway describes a folder the pane is not showing.
    ...
    private var chromedContent: some View {
        ...
        mainContentView
            ...
            // 44 (`ActionBarMetrics.height` 28 inside 2×8pt of padding), so a 560pt window leaves
            // the file list on the order of 430 — enough to be a list rather than a peephole. (That
            // is the two fixed rungs subtracted, not a rendered measurement of the list itself.)
            // It also clears the 428pt below which `SettingsLayout` stops shrinking its sheet
    ...
    var tidyScanRootExpanded: String {
        PaneLogic.tidyScanRoot(
            ...
            // it here made the "Scan '<folder>'" offer name — and scan — a folder that pane was not
            // showing. The tree lists its scope whole, so its scope is what a scan of it walks.
            browsePath: paneDrawsColumns(isLeft: !tidyTargetIsRight)
