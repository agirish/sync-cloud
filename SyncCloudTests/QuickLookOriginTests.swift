...
@Suite struct QuickLookOriginTests {
    ...
    static func callSites() throws -> [String] {
        var sites: [String] = []
        for file in ["ContentView.swift", "ContentView+SplitLayout.swift"] {
    ...
    @Test func testEveryPaneEntryPointFollowsTheSelection() throws {
        ...
        for handler in split.components(separatedBy: ".onKeyPress(.space)").dropFirst() {
            let body = String(handler.prefix(500))
            guard body.contains("toggleQuickLook") else { continue }
            #expect(body.contains("followsPane: true"),
