...
| **Several assertions about one object, every one `nil` on the left**, immediately after a wait helper that returns `Void` | [2](flaky-tests.md#2-fixed-pumps-and-fixed-sleeps) — see *A bound whose expiry is discarded* | The state never arrived; the wait expired and said nothing. Do not debug the object. Make the bound's expiry a labelled failure |
| The failing expectation is the test's **own vacuity control** (`samples > N`, "too few to have observed…") while every substantive assertion **passes** | [17](flaky-tests.md#17-the-control-that-stops-an-absence-being-vacuous-is-itself-load-dependent--fixed) | Do **not** lower the threshold — that restores the silent pass it exists to prevent. Re-express it in something the code produces, not the scheduler |
| A pasteboard (or other daemon-brokered) assertion whose actual value names a fixture **this process never created** — a foreign UUID in the path | [18](flaky-tests.md#18-a-named-nspasteboard-is-machine-global-so-two-test-processes-share-it) | Was another test process alive on this machine — CI beside a local run? Suffix the shared name with the pid |
...
| A **rendering/pixel** assertion, deterministic delta, red in the full package and green under `--filter`, after any suite gained a test that focuses a text field | [16](flaky-tests.md#16-text-field-focus-is-arbitrated-process-wide-so-any-suite-can-evict-anothers-field-editor--fixed) | Diff the two bitmaps and find the bounding box before trusting the failure message — focus is process-wide here, so the delta is often a field editor, not the subject |
...
| The same shape on **green** runs too — a window where **no** app-target test completes, then hundreds together — and one test red at a **seconds-only** wait's bound | [2](flaky-tests.md#2-fixed-pumps-and-fixed-sleeps) — see *A third spelling* | `sample` the test host inside the window before blaming `cfprefsd`: a busy main thread in other suites' test bodies is main-actor saturation. Floor the wait in turns |
