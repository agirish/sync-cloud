...
two that produce no failure at all: mechanism 8 hangs the run instead, and mechanism 10 makes a test
...
**A flush marker is NOT an eviction guard, and six of the ten files here only flush.** The common
idiom —
...
`Modules/Events/Sources/Events/Logger.swift:355-366`). But visibility is not survival. The marker is
...
**See.** `Modules/Events/Sources/Events/Logger.swift:376` for the cap and `:166` for the shared
